...
Macro.add('idp', {
  handler () {
    // if (this.args[0] === null) return this.error('Needs to be called with an argument...')
    let output = ''
    if (this.args[0] === undefined) return $(this.output).wiki(' ')
    if (typeof this.args[0] === 'array' && this.args[0].length === 0) return $(this.output).wiki(' ')
    if (this.args.length > 1) {
        if (this.args[0]) {
            $(this.output).wiki(this.args[1])
        }
    // } else {
        ...
        //     }
        // }
        if (!output) return $(this.output).wiki(' ')
        return $(this.output).wiki(output)
    }
