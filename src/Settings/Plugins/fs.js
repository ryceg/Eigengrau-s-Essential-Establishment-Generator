...
(function () {
  ...
  function saveToState (objOrStr, varName) {
    try {
      if (varName.slice(0, 14) === 'State.metadata') {
        alert('Success!')
  ...
  function loadFromFile (e) {
    ...
    const storyVar = e.storyVar || '$fileData'
    const dataType = e.dataType || 'text'
    ...
    $(reader).on('load', function (event) {
      try {
        ...
        const data = readData(dataType, target.result)
  ...
  function createImportButton (text, storyVar, dataType, asLink) {
    ...
    const $input = $(document.createElement('input'))
      .attr({
        ...
        const ev = Object.assign(clone(e), {
          storyVar,
  ...
  function importFromFile (targetOnPage, text, storyVar, dataType, asLink) {
    const $importElement = createImportButton(text, storyVar, dataType, asLink)
    let target
  ...
  Macro.add('import', {
    handler () {
      ...
      let linkText = this.args[2]
      const storyVar = this.args[0]
      let dataType = this.args[1]

      if (!linkText || typeof linkText !== 'string') {
        linkText = setup.fileSystem.config.defaultText
      }
      if (!storyVar || typeof storyVar !== 'string' || (storyVar[0] !== '$' && storyVar[0] !== '_' && storyVar.slice(0, 14) !== 'State.metadata')) {
