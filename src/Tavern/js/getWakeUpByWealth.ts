...
export const getWakeUpByWealth = (tavern: Tavern): string => {
  ...
  if (tavern.wealth === 'poor') {
    return [
      ...
      // @ts-ignore
      `You smell something burning, and make your way down to check; when you get down to the kitchen of ${tavern.name}, you find ${setup.profile(bartender, bartender.firstName)} sitting, chewing angrily over a bowl of burnt porridge.`
    ].random() as string
  }
