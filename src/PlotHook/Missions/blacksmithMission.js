const missionsSmithy = {
  ...
  testSubject () {
    return "I'm in need of someone to test out this new <<print $smithy.weapons.random()>> in a duel against me. Would you be interested in trying this? I would compensate you for the trouble of course."
  },
  bandits (town) {
  ...
  disappearingShipment () {
    return 'I had a shipment of <<print $smithy.weapons.random()>>s coming, but it never arrived... I suspect that something happened to it. Would you be interested in looking into it?'
  },
  badWeapons (town) {
    const npc = setup.createNPC(town, { hasClass: false, profession: 'blacksmith' })
    return `I received a shipment of <<print $smithy.weapons.random()>>s, but they were completely unusable- my friend <<profile \`$npcs[${JSON.stringify(npc.key)}]\`>> would never send me bad goods, so I suspect that something is amiss!`
  },
  deliverWeapons (town) {
    const npc = setup.createNPC(town, { hasClass: false, profession: 'noble' })
    return `I recently forged a <<print $smithy.weapons.random()>> for a local ${setup.profile(npc, 'noble')}, but I don't have time to bring it to ${npc.himher}. Would you be willing to take it to ${npc.himher} for me? I would pay you for your troubles of course.`
  }
}
