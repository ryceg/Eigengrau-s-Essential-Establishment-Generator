setup.graveStone = {
  create (town) {
    ...
    const material = Object.values(gravestoneData.material).random()
    ...
    grave.sentenceStrings = [
      ...
      `${['a small', 'an average sized', 'a modestly sized'].random()}, ${material.secondaryDescriptors.random()}, ${material.type.random()} grave in the shape of ${lib.articles.output(grave.shapeMedium)}. ${[`The grave has the name "${setup.profile(owner)}" ${material.iconPlacement.random()} onto it and nothing else.`, `${['Near the top', 'Near the bottom', 'In the middle'].random()} of the grave, ${material.iconPlacement.random()} onto it are the words "${['Here lies', 'R.I.P.', 'Here is burried'].random()} ${setup.profile(owner)}. ${grave.gravePhrases}."`].random()}${['', '', `${['  Just above the writing', '  Just below the writing', ' On the other side'].random()}, ${material.iconPlacement.random()} onto the grave is an image of ${lib.articles.output([grave.graveImages, grave.weaponType].random())}.`].random()}`
    ].random()
