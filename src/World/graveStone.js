setup.graveStone = {
  create (town) {
    ...
    const grave = {
      shapeSmall: gravestoneData.shapeSmall.random(),
      shapeMedium: gravestoneData.shapeMedium.random(),
      weaponType: gravestoneData.weaponType.random(),
      gravePhrases: gravestoneData.gravePhrases.random(),
      graveImages: gravestoneData.graveImages.random()
    }
    const owner = setup.createDeadNPC(town)
    const graveMaterial = lib.keys(gravestoneData.material).random()
    ...
    grave.sentenceStrings = [
       ...
       `a small pile of earth with ${lib.articles.output(gravestoneData.material.metal.secondaryDescriptors.random())} ${gravestoneData.material.metal.type.random()} ${grave.weaponType} stuck into it. ${['A long forgotten solider likely lays here', 'A mighty fallen warrior was likely laid to rest here', 'Surely a strong fighter was laid to rest here', 'Here lays a hero who fell in battle, their name is now forgotten'].random()}.`,
       `${['a small', 'an average sized', 'a modestly sized'].random()}, ${gravestoneData.material[graveMaterial].secondaryDescriptors.random()}, ${gravestoneData.material[graveMaterial].type.random()} grave in the shape of ${lib.articles.output(grave.shapeMedium)}. ${[`The grave has the name "${setup.profile(owner)}" ${gravestoneData.material[graveMaterial].iconPlacement.random()} onto it and nothing else.`, `${['Near the top', 'Near the bottom', 'In the middle'].random()} of the grave, ${gravestoneData.material[graveMaterial].iconPlacement.random()} onto it are the words "${['Here lies', 'R.I.P.', 'Here is burried'].random()} ${setup.profile(owner)}. ${grave.gravePhrases}."`].random()}${['', '', `${['  Just above the writing', '  Just below the writing', ' On the other side'].random()}, ${gravestoneData.material[graveMaterial].iconPlacement.random()} onto the grave is an image of ${lib.articles.output([grave.graveImages, grave.weaponType].random())}.`].random()}`
    ].random()
