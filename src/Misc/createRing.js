const createRingData = () => ({
  ...
  activation: {
    'on a particular plane': `when worn in ${[
        'the celestial realm', 'the fiendish realm', 'the mortal world', 'the realm of death', 'the realm of dreams and magic', 'the realm of shadow and death', 'the ethereal plane', 'the astral plane', 'the elemental planes', 'the air elemental plane', 'the fire elemental plane', 'the earth elemental plane', 'the water elemental plane', 'the Upper planes', 'the Lower planes', 'Elysium', 'the Beastlands', 'Bytopia', 'Mount Celestia', 'Arborea', 'Arcadia', 'Ysgard', 'Mechanus', 'Limbo', 'Acheron', 'the Nine Hells', 'Gehenna', 'Hades', 'Carceri', 'the Abyss', 'Pandemonium'
      ].random()}`,
...
setup.createRing = base => {
  const ringData = createRingData()

  const ring = {
    power: Object.values(ringData.power).random(),
    cost: Object.values(ringData.cost).random(),
    activation: Object.values(ringData.activation).random(),
    ...base
  }
