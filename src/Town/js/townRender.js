setup.townRender = function (town) {
  ...
  if (town.roll.economics > 90) {
    town.economics = `Trade in ${town.name} is heavily regulated, with taxes, tariffs, and restrictions on what can be brought in and out of the ${town.type}, and people live ${setup.articles.output(town.wealth)} existence because of it. The trade guild strictly enforces the rules, and costs of doing business in ${town.name} are high.`
  } else if (town.roll.economics > 80) {
    town.economics = `Trade in ${town.name} is regulated, with taxes and restrictions on what can be brought in and out of the ${town.type}, and people live ${setup.articles.output(town.wealth)} existence because of it. The trade guild enforces rules, with stiff penalties and trade bans for rule-breakers.`
  } else if (town.roll.economics > 70) {
    town.economics = `Trade in ${town.name} is regulated, with taxes applied to all goods and services rendered, and people live ${setup.articles.output(town.wealth)} existence because of it. The trade guild enforces rules, with penalties for rule-breakers.`
  } else if (town.roll.economics > 60) {
    town.economics = `Trade in ${town.name} is mostly free, with some taxes applied to goods and services rendered in the city. People live ${setup.articles.output(town.wealth)} existence because of it.`
  } else if (town.roll.economics > 50) {
    town.economics = `Trade is reasonable in ${town.name}, and people live ${setup.articles.output(town.wealth)} existence because of it; some taxes are applied to certain goods and services that are rendered in the city.`
  } else if (town.roll.economics > 40) {
    town.economics = `Trade is reasonable in ${town.name}, and people live ${setup.articles.output(town.wealth)} existence because of it; some taxes are applied to certain goods and services that are rendered in the city, but the more creative entrepenuers can find loopholes to make a better profit.`
  } else if (town.roll.economics > 30) {
    town.economics = `Trade is rather free in ${town.name}, and people live ${setup.articles.output(town.wealth)} existence because of it. There are few taxes, and there is little regulation from the authorities on what merchants can and cannot sell.`
  } else if (town.roll.economics > 20) {
    town.economics = `Trade is free in ${town.name}, and people live ${setup.articles.output(town.wealth)} existence because of it. There are no taxes or regulations to speak of.`
  } else if (town.roll.economics <= 20) {
    town.economics = `Caveat emptor is the guiding philosophy of ${town.name}, and people live ${setup.articles.output(town.wealth)} existence because of it. Without any taxes or regulations, the free market reigns supreme here.`
  }
  ...
  if (town.roll.military > 90) {
    ...
    town.military = 'The military is extremely strict, with citizens being forced to carry licenses, identification papers, and travel permits. The local guard, <<profile $town.guard>>, is extremely well armed, and brutality is commonplace.'
  } else if (town.roll.military > 80) {
    town.roll.guardFunding += 50
    town.military = 'The military is very strict, with citizens being forced to carry licenses and travel permits. The local guard, <<profile $town.guard>>, is well armed, and brutality is common.'
  } else if (town.roll.military > 70) {
    town.roll.guardFunding += 40
    town.military = 'There is a strong military presence, with citizens seeking to live in the city being forced to undergo background checks. The local guard, <<profile $town.guard>>, is well armed, and brutality is not unheard of.'
  } else if (town.roll.military > 60) {
    town.roll.guardFunding += 30
    town.military = "There's a decent military presence, and citizens know better than to step out of line, as <<profile $town.guard>> are armed. Despite this, brutality is rare, and citizens with nothing to hide have nothing to fear."
  } else if (town.roll.military > 50) {
    town.roll.guardFunding += 30
    town.military = `There is a military presence in ${town.name}, but it is more for outside intruders rather than the population inside. Citizens are expected to follow the laws, and those that do have nothing to fear from <<profile $town.guard>>.`
  } else if (town.roll.military > 40) {
    town.roll.guardFunding += 25
    town.military = `The military presence in ${town.name} is somewhat smaller than one would expect for its size. The local guard, <<profile $town.guard>>, are seen as friends and drinking buddies, rather than horrible tyrants.`
  } else if (town.roll.military > 30) {
    town.roll.guardFunding += 25
    town.military = `The military presence in ${town.name} is more of a militia; some of the members of <<profile $town.guard>> are part time, and there is little need for the use of force outside of intruders.`
  } else if (town.roll.military > 20) {
    town.roll.guardFunding += 15
    town.military = `The military presence in ${town.name} is a militia; most of the guards of <<profile $town.guard>> are part time, and there is little need for the use of force. Citizens act with honesty, and have no need for 24/7 guards.`
  } else if (town.roll.military <= 20) {
    town.roll.guardFunding += 10
    town.military = `The military presence in ${town.name} is a militia; the guards of <<profile $town.guard>> are part time, and there is rarely any need for the use of force. Law breakers are judged by community tribunals.`
  }
