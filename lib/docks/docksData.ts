...
// function makeDocksData<T extends X> () {
export const docksData = {
  ...
  ships: {
    create (town: Town, docks: Docks, opts: Partial<Ship>) {
      const ship = Object.assign({
        name: random(lib.docksData.ships.name),
        // type: random(Object.keys(lib.docksData.ships.type)) as ShipTypes,
        captainType: random(Object.keys(lib.docksData.ships.captain)),
        hull: random(lib.docksData.ships.hullDesc),
        detail: random(lib.docksData.ships.shipDetail),
        event: random(lib.docksData.ships.eventDetail),
        roll: {
          size: random(1, 100),
      ...
      // Object.assign(ship, lib.docksData.ships.type[ship.type].hasOars, lib.docksData.ships.type[ship.type].rigging)
      // Object.assign(ship, {
      //   crewMen: lib.docksData.ships.type[ship.type].crewMen(),
      //   masts: lib.docksData.ships.type[ship.type].masts(),
      //   length: lib.docksData.ships.type[ship.type].length(),
      //   purpose: random(lib.docksData.ships.type[ship.type].purpose)
      // })
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      ship.captain = setup.createNPC(town, lib.docksData.ships.captain[ship.captainType])

      const rollDataVariables = ['size', 'cleanliness'] as const
      for (const propName of rollDataVariables) {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        lib.defineRollDataGetter(ship, lib.docksData.ships.rollData[propName].rolls, propName)
      }
      docks.ships[ship.name] = ship as Ship
      return ship
    },
    crew: {
      create (town: Town) {
        const crewType = random(lib.docksData.ships.crew.type)

        const readout = {
          bodyFeature: random(lib.docksData.ships.crew.bodyFeature),
          itemFeature: random(lib.docksData.ships.crew.itemFeature),
          sailReason: random(lib.docksData.ships.crew.sailing)
        }
        const crewTraits = Object.assign({
          profession: 'sailor'
        }, crewType)
        const crew = setup.createNPC(town, crewTraits as Partial<NPC>)
      ...
      type: [
        {
          type: 'a veteran of the sea who may have been beautiful at one point',
          gender: 'woman',
          note: 'A woman whose skin has been brined by the salt water.'
        },
        {
          type: 'a young sailor excited to take to the sea',
          ageStage: 'young adult',
          calmTrait: 'passionate'
        },
        {
          type: 'a wide eyed adventurer of the sea just getting a first true taste of sailing',
          ageStage: 'young adult',
          calmTrait: 'curious',
        ...
        {
          type: 'an older, well-brined master of sailing',
          ageStage: 'elderly'
        },
        { type: 'an ordinary looking sailor' },
        {
          type: 'a stoic looking veteran',
          ageStage: 'settled adult',
          calmTrait: 'quiet',
          stressTrait: 'reliable'
        },
        {
          type: 'a rugged and battered looking bilge rat',
          note: 'This person has extremely dirty clothing on.'
        },
        {
          type: 'a suspiciously regal and seasick looking sailor',
          profession: 'sailor',
    ...
    customers: [
      {
        ...
        description (building: Docks, npc: NPC) { return `${npc.firstName} works with ships.` }
      },
      {
        ...
        description (building: Docks, npc: NPC) { return `${npc.firstName} constructs and repairs ships in ${building.name}.` }
      },
      {
        ...
        description (building: Docks, npc: NPC) { return `${npc.firstName} works to organise equipment to be loaded off ${building.name} and onto ${npc.hisher} ship.` }
      },
      {
        ...
        description (building: Docks, npc: NPC) { return `${npc.firstName} hauls stuff off ships and onto ${building.name}.` }
      }
    ]
  }
}
...
// export const docksData = makeDocksData()

export type ShipTypes = keyof typeof docksData.ships.type
// export type Locations = UnionKeys<ShipTypes[keyof ShipTypes]['location']>
