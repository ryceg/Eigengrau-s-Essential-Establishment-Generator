setup.getNPCProfession = function(profession) { // I'm not really sure how
  ...
  if (professionCount >= 1){ // If there's at least one person
      if (State.variables.NPCArray[0].profession.count < professionCount){ // Test to see if the number of people that match that profession in the indexed array is less than the number that should be created
          creationInstances = (professionCount -= State.variables.NPCArray[0].profession.count); // if there are less people, define the deficit
      ...
      else if (State.variables.NPCArray[0].profession.count > professionCount){ // if there's more than there should be
        while (State.variables.NPCArray[0].indexOf(profession) !== professionCount) { //splice them outta the array.
              State.variables.NPCArray[0].splice(State.variables.NPCArray[0].indexOf(profession), 1); /* Using splice removes the item and reindexes the array. This might cause problems further down the line. */
        }
      }
  }
};
